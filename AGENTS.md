...
- **一键隐藏联动面板**：视图 `when` 条件为上下文键 `watchStock.show`，激活时设为 true，老板键隐藏时设为 false 使 `WATCH-STOCK` tab 从面板中消失（不执行 `closePanel`，避免误关终端）。`autoHideByMarket` 只作用于状态栏，不影响面板。用正向键而非 `!hidden`，是为了激活前视图不存在，防止启动时恢复面板直接弹出行情。

...
- 📋 **查看股票** 底部面板查看自选、指数、板块、分时图和成交量等
- ⏰ **价格闹钟** 设置价格提醒，价格达到目标自动通知
- 🔒 **封单监控** 监控涨跌停股票封单数量，异动自动通知
- 💰 **大单监控** 监控股票区间内成交额异动，自动通知大单买卖
- 🔄 **灵活排序** 支持自定义排序，或按涨跌幅升序/降序排列
- 👁️ **一键隐藏** 老板键一键隐藏状态栏与股票面板
...
4. **查看股票**：点击状态栏股票信息 → 选择"查看股票"，或直接打开底部面板的 `WATCH-STOCK` 标签
   - 「自选」列出已添加的股票，点击某行展开详细指标、分时图和成交量，再次点击收起
   - 「指数」查看大盘指数，「板块」查看行业板块涨跌
   - 面板可见时行情自动刷新；可将面板拖到侧边栏
5. **价格闹钟**：点击状态栏股票信息 → 选择"价格闹钟"
   - 设置新闹钟：选择股票 → 选择条件（高于/低于）→ 输入目标价格
   - 管理闹钟：在闹钟列表中点击即可删除，或选择"删除所有闹钟"
6. **一键隐藏**：点击状态栏股票信息，选择"一键隐藏"或"恢复显示"
   - 隐藏时状态栏与底部面板的 `WATCH-STOCK` 标签同时消失
   - 使用快捷键：`Ctrl+Alt+S`（Windows/Linux）或 `Cmd+Alt+S`（macOS）
   - 开休市自动显隐：配置`autoHideByMarket`为`true`，即可根据当前时间自动显示/隐藏状态栏（仅作用于状态栏）
...
| `enableColorful`   | boolean | `false`        | 股票面板中涨跌以红绿色显示（状态栏不受影响）  |

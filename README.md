...
4. **查看股票**：点击状态栏股票信息 → 选择"查看股票"
   - 默认展示 `overview` 页签，可查看大盘指数与行业板块涨跌（行业按涨幅排序）
   - 点击股票标签切换到个股详情：ASCII 分时走势（含成交量直方图、最高/最低价 `▲`/`▼` 标注）与今开/最高/最低/昨收、成交额、市盈市净等指标
   - 面板整体为终端文本风格：等宽字体、`❯` 提示符，涨跌只用 `+`/`-` 与数值表达，旁人视角下如同日志输出
   - 面板较窄时，标签栏、分时图与指数表各自横向滚动，不会挤压变形
...
# 安装依赖
npm install
# 类型检查（查看面板是独立编译的浏览器端工程，改 src/webview/ 后同样走这条检查）
npm run typecheck
# 使用 VS Code 打开项目
# 按 F5 启动调试模式

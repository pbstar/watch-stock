 ...
 * @param {number} datalen - 数据条数,默认240(4小时)
 * @returns {Promise<Array>} 分时数据数组
 */
async function getStockMinuteData(code, datalen = 240) {
  if (!code) {
    return [];
  }
  ...
  try {
    const response = await httpGet(
      `http://money.finance.sina.com.cn/quotes_service/api/json_v2.php/CN_MarketData.getMinkLineData?symbol=${code}&scale=1&datalen=${datalen}&ma=no`,
      {
        timeout: 5000,
        responseType: "arraybuffer",
        headers: {
    ...
    console.log(`获取分时数据成功: ${result.length} 条`);
    return result;
  } catch (error) {
    const errorMsg = `获取分时数据失败: ${error.message}`;
    console.error(errorMsg);
    vscode.window.showErrorMessage(errorMsg);
    return [];
  }
